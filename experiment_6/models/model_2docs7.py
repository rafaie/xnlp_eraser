...
@Model.register("model_2docs7")
class Model2Docs7(BaseModel):
    def __init__(
        self,
        vocab: Vocabulary,
        ...
        super(Model2Docs7, self).__init__(
    ...
    def forward(self, document, kept_tokens, rationale=None, label=None, metadata=None) -> Dict[str, Any]:
        ...
        attentions = self.cross_att(query_states=premise_text,
                                       attention_mask=premise_mask,
                                       source_states=query_text,
                                       source_attention_mask=query_mask)
        
        embedded_text = premise_text * attentions.hidden_states.sum(-1).unsqueeze(-1) * premise_mask.unsqueeze(-1)
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        base_metrics = super(Model2Docs7, self).get_metrics(reset)
