...
class Objective2DocsModel(BaseModel):
    def __init__(
        ...
        super(Objective2DocsModel, self).__init__(vocab, initializer, regularizer)
        ...
        self._premise_vector = torch.nn.Parameter(torch.randn((1, self._premise_seq2seq_encoder.get_output_dim())))
        self._query_vector = torch.nn.Parameter(torch.randn((1, self._query_seq2seq_encoder.get_output_dim())))
    ...
    def forward(self, premise_sample_z, query_sample_z, label=None, metadata=None) -> Dict[str, Any]:
        ...
        # premise
        premise = self._regenerate_tokens(sample_z=premise_sample_z, metadata=metadata)
        ...
        # query
        query = self._regenerate_tokens(sample_z=query_sample_z, metadata=metadata, is_premise=False)
        ...
        embedded_text = torch.cat((premise_embedded_text, query_embedded_text), 1)
        ...
        if label is not None:
            if isinstance(label, list):
                ...
                label = torch.LongTensor([output_labels[l["Label"]] for l in label]).to(logits.device)
    ...
    def _regenerate_tokens(self, metadata, sample_z, is_premise=True):
        ...
        tokens = [m["premise_tokens" if is_premise is True else "query_tokens"] for m in metadata]
